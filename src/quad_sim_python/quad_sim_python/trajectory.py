...
class Trajectory:
    ...
    def __init__(self, curr_heading, ctrlType, trajSelect, trajPoints, dist_consider_arrived=0.2):
        ...
        t_wps, wps, y_wps, v_wp = trajPoints
        self.t_wps = t_wps
        self.wps   = wps
        self.y_wps = y_wps
        self.v_wp  = v_wp

        self.dist_consider_arrived = dist_consider_arrived # Distance to waypoint that is considered as "arrived"
        ...
        def pos_waypoint_arrived_wait():
            ...
            # If end is not reached, calculate distance to next waypoint
                ...
                # Replace 'self.t_wps[self.t_idx]' by any number to have a fixed waiting time for all waypoints
                    ...
                    if (self.t_idx >= (len(self.wps[:,0])-1)):    
